...
def load_data(dataset='HIV'):
    ...
    return feats_tensor, weighted_adjs_tensor, instances_labels, train_val_test_idx
...
def get_loc_feats_DC():
    raw_stations_name2id = np.load('../data/preprocessed/BikeDC/raw_stations_name2id.npy', allow_pickle=True)[()]
    filename = '../data/raw/BikeDC/2020/202012.csv'
    csv_file = pd.read_csv(filename)
    raw_stations_feats = csv_file[csv_file['start_station_name'].isin(raw_stations_name2id)]
    raw_stations_feats = raw_stations_feats[['start_station_name', 'start_lat', 'start_lng']].drop_duplicates(subset=['start_station_name'], keep='first')
    raw_stations_feats = raw_stations_feats.replace({"start_station_name": raw_stations_name2id})
    raw_stations_feats = raw_stations_feats.sort_values(by="start_station_name")
    raw_stations_feats = raw_stations_feats[['start_lat','start_lng']].to_numpy()
    np.save('../data/preprocessed/BikeDC/raw_stations_feats.npy',raw_stations_feats)
...
# optional
def res_regions_DC():
    res_regions = np.load('../data/preprocessed/BikeDC/res_regions.npy',allow_pickle=True)[()]
    raw_stations_name2id = np.load('../data/preprocessed/BikeDC/raw_stations_name2id.npy',allow_pickle=True)[()]
    idold2new = {}
...
def generate_adj_tensor_00_DC(view_list, years_list, quarters_list,months_list):
    # if use res regions
    # n_num = len(np.load('../data/preprocessed/res_regions_name2id.npy',allow_pickle=True)[()].keys())
    n_num = np.load('../data/preprocessed/BikeDC/raw_stations_feats.npy',allow_pickle=True).shape[0]
    ...
    for i, year in enumerate(years_list):
        if i < 3:
            for quarter in quarters_list:
                ...
                print(filename)
                csv_file = pd.read_csv(filename)
                months = [i + (int(quarter.split('Q')[-1]) - 1) * 3 for i in range(1, 4)]
                months = ['0' + str(month) if month < 10 else str(month) for month in months]
                for month in months:
                    generate_adj_tensor_01_DC(year, month, csv_file,example_id, BikeDC_adj_tensor,n_num)
                    example_id += 1
        else:
            for month in months_list:
                filename = '../data/raw/BikeDC/' + year + month + '.csv'
                ...
                generate_adj_tensor_01_DC(year, month, csv_file, example_id, BikeDC_adj_tensor, n_num)
                example_id += 1
    np.save('../data/preprocessed/BikeDC/BikeDC_adj_tensor.npy',BikeDC_adj_tensor)
    np.save('../data/preprocessed/BikeDC/BikeDC_feat_tensor.npy',BikeDC_adj_tensor)


def generate_adj_tensor_01_DC(year, month, csv_file, example_id,BikeDC_adj_tensor, n_num):
    # if use res regions
    ...
    if year_ == '2020' and int(month) >= 4:
        ...
        s_time = 'Start date'
    weekdays = np.array(calendar.Calendar(calendar.MONDAY).monthdatescalendar(int(year_), int(month))[1:]).transpose()[:5].reshape(-1)
    weekends = np.array(calendar.Calendar(calendar.SATURDAY).monthdatescalendar(int(year_), int(month))[1:]).transpose()[:2].reshape(-1)
    weekdays = [m.strftime('%Y-%m-%d') for m in weekdays]
    weekends = [s.strftime('%Y-%m-%d') for s in weekends]
    days_list = [len(weekdays), len(weekends),calendar.monthrange(int(year_), int(month))[1]]
    print(year_month, end='')
    print(days_list)
    target_elems = csv_file[csv_file[s_time].str.contains(year_month)]
    target_elems = target_elems[target_elems[s_station].isin(raw_stations_name2id)].replace({s_station: raw_stations_name2id})
    target_elems = target_elems[target_elems[e_station].isin(raw_stations_name2id)].replace({e_station: raw_stations_name2id})
    ...
    generate_adj_tensor_02_DC(BikeDC_adj_tensor, n_num, example_id, weekday_elems, weekend_elems, month_elems,s_station, e_station, days_list)


def generate_adj_tensor_02_DC(BikeDC_adj_tensor, n_num, example_id,weekday_view,weekend_view, month_view,s_station, e_station, days_list):
    for i, (start, end) in enumerate(zip(weekday_view[s_station], weekday_view[e_station])):
        BikeDC_adj_tensor[example_id][0][start][end] += 1
        BikeDC_adj_tensor[example_id][0][end][start] += 1
    for i, (start, end) in enumerate(zip(weekend_view[s_station], weekend_view[e_station])):
        BikeDC_adj_tensor[example_id][1][start][end] += 1
        BikeDC_adj_tensor[example_id][1][end][start] += 1
    for i, (start,end) in enumerate(zip(month_view[s_station],month_view[e_station])):
    ...
    for i, num_days in enumerate(days_list):
        BikeDC_adj_tensor[example_id][i] = BikeDC_adj_tensor[example_id][i] / num_days
        for j in range(n_num):
            BikeDC_adj_tensor[example_id][i][j] = BikeDC_adj_tensor[example_id][i][j] / np.sum(BikeDC_adj_tensor[example_id][i][j])


def label_regions_DC():
    raw_stations_feats = np.load('../data/preprocessed/BikeDC/raw_stations_feats.npy')
    central_point = np.mean(raw_stations_feats, axis=0, keepdims=True)
    central_point = np.repeat(central_point,axis=0,repeats=len(raw_stations_feats))
    distances = raw_stations_feats - central_point
    distances = np.linalg.norm(distances, axis=1)
    sorted_indices = np.argsort(distances)
    l0 = sorted_indices[:math.ceil(len(distances) / 4)]
    l1 = sorted_indices[math.ceil(len(distances) /4):math.ceil(len(distances) / 2)]
    l2 = sorted_indices[math.ceil(len(distances) /2):math.ceil(len(distances) * 3 / 4)]
    ...
    np.save('../data/preprocessed/BikeDC/BikeDC_reg_labels.npy',BikeDC_reg_labels)
...
def split_train_val_test(dataset, ratio_list):
    if dataset == 'HIV' or dataset == 'BP':
        ...
        raw_data = np.load('../data/preprocessed/PROTEINS/PROTEINS_ins_labels.npy')
...
def load_raw_data_PROTEINS(dim_ins, dim_node):
    ...
    with open('../data/raw/PROTEINS/PROTEINS_node_attributes.txt') as f:
        ...
        for node_idx, node_feats in enumerate(txt_file[:-1]):
            node_idx2feats[node_idx] = np.array(list(map(float, node_feats.split(','))))
    ...
    with open('../data/raw/PROTEINS/PROTEINS_A.txt') as f:
        ...
        num_ins = 0
        PROTEINS_adj_tensor = np.empty((len(ins_idx2edges.keys()), 2, dim_ins, dim_ins))
        PROTEINS_feat_tensor = np.empty((len(ins_idx2edges.keys()), 2, dim_ins, dim_ins))
        PROTEINS_reg_labels = np.empty((len(ins_idx2edges.keys()), dim_ins),dtype=np.int)
        PROTEINS_ins_labels = np.empty((len(ins_idx2edges.keys()),),dtype=np.int)
        for ins_idx, ins_edges in ins_idx2edges.items():
            ...
            if len(node_idxes) <= dim_ins and num_ins < 1000:
                ...
                PROTEINS_reg_labels[num_ins] = np.array(list(node_idx2label[node_idxes]) + [3] * (dim_ins - len(node_idxes)))
        ...
        np.save('../data/preprocessed/PROTEINS/PROTEINS_adj_tensor.npy', PROTEINS_adj_tensor)
        np.save('../data/preprocessed/PROTEINS/PROTEINS_feat_tensor.npy', PROTEINS_feat_tensor)
        np.save('../data/preprocessed/PROTEINS/PROTEINS_reg_labels.npy', PROTEINS_reg_labels)
        np.save('../data/preprocessed/PROTEINS/PROTEINS_ins_labels.npy', PROTEINS_ins_labels)


def generate_adj_tensor_PROTEINS(dim_ins, node_idxes, ins_edges,node_idx2feats):
    node_old2newidx = {}
    node_idx_new = 0
...
def Normalization(X):
    min_values = np.min(X, axis=(-2, -1), keepdims=True)
    max_values = np.max(X, axis=(-2, -1), keepdims=True)
    X = (X - min_values) / (max_values - min_values)
    return X


def spectral_embedding_combined_views(combined_X, n_components):
    embedding = SpectralEmbedding(n_components=n_components, affinity='nearest_neighbors')
    X_transformed = embedding.fit_transform(combined_X)
...
def CorEn(dataset, X):
    ...
    if dataset == 'HIV':
        percentile = np.percentile(X[:, 0, :, :], 90)
        X[:, 0, :, :] = np.clip(X[:, 0, :, :], None, percentile)
        X = Normalization(X)

    mul_X = np.zeros((V, N, D))
    for i in range(V):
        temp = np.matmul(X[:, i, :, :], X[:, i, :, :].transpose(0, 2, 1))
        mul_X[i] = temp.sum(axis=0)
    mul_X = Normalization(mul_X)
    combined_X = np.concatenate(mul_X, axis=1)
    multi_features = spectral_embedding_combined_views(combined_X, 35)
    u, _, v = np.linalg.svd(multi_features)
    G = np.matmul(u.T.reshape(1, 1, D, N), X)

    return G, [u, v]


def tensor_enhancement(dataset, init_adjs_tensor):
    G, _ = CorEn(dataset, init_adjs_tensor)

    M, V, N, N = init_adjs_tensor.shape
    adj_tensor = np.zeros((M, V, N, N))
    feat_tensor = np.zeros((M, V, N, N))
    ...
    for m in range(M):
        for v in range(V):
            ...
            important_indices = np.argpartition(imp_matrix, -int(N * 0.2), axis=1)[:, -int(N * 0.2):]
            rows = np.tile(np.arange(N)[:, np.newaxis], (1, int(N * 0.2)))
            tmp_matrix[rows, important_indices] = 1

            tmp_matrix += np.eye(N) + tmp_matrix.T
            tmp_matrix[tmp_matrix > 1] = 1

            adj_matrix = np.where(tmp_matrix == 1, 0.15, 0)
            np.fill_diagonal(adj_matrix, 1)
            adj_tensor[m, v] = adj_matrix

            differences = G[m, v, :, np.newaxis, :] - G[m, v, np.newaxis, :, :]
            norms = np.linalg.norm(differences, axis=-1, ord=2)
            feat_tensor[m, v] = np.where(tmp_matrix == 1, np.exp(-0.1 * norms), 0)

    return feat_tensor, adj_tensor
...
if __name__ == '__main__':
    ...
    # The ogbg-hiv ACM has been processed
