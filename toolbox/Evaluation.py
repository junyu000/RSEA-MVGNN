...
def evaluate_results_nc(embeddings, labels, num_classes):
    ...
    print('SVM:Macro-F1: ' + ', '.join(['{:.6f} ({:.1f})'.format(macro_f1, train_size)
                                        for macro_f1, train_size in zip(svm_macro_f1_list, [0.6, 0.2])]))
    print('SVM:Micro-F1: ' + ', '.join(['{:.6f} ({:.1f})'.format(micro_f1, train_size)
                                        for micro_f1, train_size in zip(svm_micro_f1_list, [0.6, 0.2])]))
